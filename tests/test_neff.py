...
def independent_pool(K=60, M=400, seed=0):
    """Candidates that share nothing: same accuracy, no item difficulty structure."""
    rng = np.random.default_rng(seed)
    return (rng.random((K, M)) < 0.7).astype(np.uint8)


def single_factor_pool(K=60, M=400, seed=0):
    """One shared item-difficulty factor drives every candidate."""
    rng = np.random.default_rng(seed)
...
def test_spectral_neff_survives_a_constant_candidate():
    """A candidate that is right on every item has zero variance and no correlation."""
...
def test_expected_max_gaussian_is_below_asymptotic_form():
    """sqrt(2 ln n) overshoots at the N values prompt search actually uses."""
    for n in (5, 10, 25, 50):
...
def test_mutation_pool_is_more_correlated_than_seed_pool():
    """The secondary result the provenance labels exist to support."""
