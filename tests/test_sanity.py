"""The sanity checks are bug detectors, so these tests inject bugs and demand a FAIL."""
...
def test_monotone_in_m_catches_a_reversed_grid():
    """Swapping two cells simulates an off-by-one in the dev-size loop."""
    broken = dict(GRID)
...
def test_monotone_checks_tolerate_pure_noise():
    """Flat grids must not trip the check just because cell means jitter."""
    flat = {
...
def test_n1_check_catches_a_leaky_truth_pool():
    """If truth were computed on dev items, N=1 bias would sit far from the split offset."""
...
def test_correction_check_treats_union_over_K_overshoot_as_a_finding():
    """Overcorrecting on the nominal candidate count is a result, not a bug."""
    results = {
        "none": {"abs_bias": 0.05},
        "split": {"abs_bias": 0.01},
