...
the same procedure above, one more `generate_candidates.py` / `build_matrix.py` pass.
...
Then run the analysis, which will not draw a figure until every sanity check
passes:
