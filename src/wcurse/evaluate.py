"""Grading the correction methods against known truth.

A method can win on bias and lose on coverage, so all three targets are reported: mean
bias, mean squared error, and interval coverage.
"""
...
def evaluate_methods(
    ...
    """Run every method on the same draws and score it against the truth pool.

    Pairing matters: all methods see the identical ``(N, m)`` dev view on a given seed, so
    the per-seed errors can be fed straight into a Wilcoxon signed-rank test.
    """
