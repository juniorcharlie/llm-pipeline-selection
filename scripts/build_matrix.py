"""Score a candidate pool against evaluation items and build the correctness matrix.

This is the one compute-bound step in the plan. Everything downstream is resampling on
the stored matrix, so the matrix is checkpointed one candidate at a time: a killed Kaggle
session costs at most a single candidate's worth of work, never the run.

Pilot first, commit second::

    # measure throughput on ten candidates, then extrapolate
    python scripts/build_matrix.py --task sst2 --limit-candidates 10 --pilot

    # the real thing, resumable: rerun the identical command after a session dies
    python scripts/build_matrix.py --task sst2
    python scripts/build_matrix.py --task sst2 --finalize

Scoring is a single constrained token. ``allowed_token_ids`` restricts sampling to the
first token of each verbalizer, so the model cannot dodge the question, and greedy decoding
makes the prediction the argmax over exactly those classes.
"""
...
def verbalizer_token_ids(tokenizer, task: Task) -> list[int]:
    """First-token id of each verbalizer, with a collision check.

    If two classes share a first token the whole scoring scheme silently collapses into a
    coin flip between them, so this fails loudly instead.
    """
    ids = []
...
def prepare_run(task: Task, model: str, item_seed: int, split_seed: int, ckpt_dir: Path) -> dict:
    """Fix the items and the dev/truth split once, then never touch them again.

    The metadata file is written on the first call and verified on every later call, so a
    resumed run cannot quietly score a different item sample than the rows already on disk.
    """
...
def score_candidates(
    ...
    """Score every candidate not already checkpointed, one candidate per file."""
...
def finalize(pool: list[dict], meta: dict, ckpt_dir: Path, out_path: Path) -> CorrectnessMatrix:
    """Assemble checkpointed prediction rows into a published correctness matrix."""
