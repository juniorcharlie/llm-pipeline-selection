"""Build the K=100 candidate pool for one task, with provenance labels.

Pool composition, following the PRD:

- 10 manual seed instructions, read verbatim from ``data/prompts/seeds.json``
- 45 APE-style candidates, each written by the model from labelled demonstrations
- 45 paraphrases and mutations of the seeds

Provenance is recorded per candidate so that pool composition becomes an independent
variable: mutation families should show a lower effective candidate count than
independently generated ones, which would mean evolutionary optimizers suffer *less*
nominal selection bias than random search at the same N.

Usage on Kaggle (needs a GPU and vLLM):

    python scripts/generate_candidates.py --task sst2 --model Qwen/Qwen2.5-1.5B-Instruct

Offline, to inspect the plumbing without a model:

    python scripts/generate_candidates.py --task sst2 --backend stub
"""
