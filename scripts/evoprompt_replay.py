...
def clean(raw: str) -> str:
    """Same cleanup rule as generate_candidates.py, kept in sync deliberately."""
...
class StubBackend:
    """Deterministic stand-in so the GA loop, replacement logic, and checkpointing can be
    exercised without a GPU. Fitness is a hash of the prompt text, not real accuracy, and
    must never be used for a trajectory that appears in the paper.
    """
...
class VLLMBackend:
    """Constrained-token scoring (same scheme as build_matrix.py) plus free-text
    crossover generation, sharing one loaded model.
    """
...
def load_run_meta(task: Task, model: str, ckpt_root: Path) -> dict:
    """Reconstruct item texts/labels/splits for this (task, model).

    Prefers the matrix itself plus a deterministic re-derivation via ``load_items`` --
    Kaggle working directories don't persist between sessions, so the checkpoint
    ``meta.json`` that ``build_matrix.py`` wrote is usually gone even though the published
    ``.npz`` (which is all this really needs) survived. Falls back to the checkpoint file
    only if it happens to still be there, as a faster offline-friendly shortcut.
    """
...
def fitness_batch(meta: dict, m: int, seed: int) -> tuple[list[str], list[int]]:
    """Fixed m-item sample from the dev pool, drawn once for the whole trajectory.

    Fixed rather than resampled per generation, because a real optimizer with a dev budget
    of m items evaluates every candidate against the same batch -- that stationarity is
    part of what we're checking the replay methodology against.
    """
...
def roulette_select(population: list[dict], rng: np.random.Generator, k: int = 2) -> list[dict]:
    """Fitness-proportional selection with a floor, so a near-zero-variance population
    (early generations, or a stub run) never divides by zero or collapses to one parent.
    """
