...
One command, no GPU, no model::

    python scripts/make_figures.py

Reads the CSVs written by ``run_analysis.py`` and writes ``figures/`` and ``tables/``.
Table 2, which places measured bias next to the gains published prompt optimizers claim,
needs those published numbers; they live in ``data/published_gains.csv`` and are left for a
human to fill in with citations rather than being guessed here.
"""
...
def load_all(results_root: Path) -> dict[str, pd.DataFrame]:
    """Concatenate the per-run CSVs into one frame per analysis product."""
...
def pick(available: pd.Series, requested: int, label: str) -> int:
    """Resolve a requested slice value against what the grid actually contains.

    The published matrices support the full grid, but pilot and test matrices do not, and a
    figure script that dies on an empty slice is a bad way to find that out. Falls back to
    the nearest available value and says so.
    """
...
def figure1_bias_vs_n(grid: pd.DataFrame, out: Path, m_fixed: int) -> None:
    """Bias against candidate count at fixed dev-set size, one panel per task."""
...
def figure2_bias_vs_m(grid: pd.DataFrame, out: Path, n_fixed: int) -> None:
    """Bias against dev-set size at fixed candidate count, with the EVT reference slope."""
    tasks = sorted(grid["task"].unique())
    fig, axes = _panels(tasks)
    for ax, task in zip(axes, tasks):
...
def figure4_spectrum(results_root: Path, neff: pd.DataFrame, out: Path) -> None:
    """Eigenvalue spectrum of the candidate correlation matrix, and N_eff by provenance."""
    ...
    for run in sorted(p for p in results_root.iterdir() if p.is_dir()):
        ...
        with np.load(path) as z:
            lam = z["all"]
        axes[0].plot(np.arange(1, lam.size + 1), lam / lam.sum(), marker=".", label=run.name.split("__")[0])
...
def figure5_calibration(methods: pd.DataFrame, out: Path, alpha: float) -> None:
    """Interval coverage by correction method against the nominal level."""
...
def figure6_allocation(alloc: pd.DataFrame, out: Path) -> None:
    """Expected true score against N at fixed total budget: the interior optimum."""
...
def write_tables(results_root: Path, data: dict[str, pd.DataFrame], out: Path, alpha: float) -> None:
    """Tables 1, 3, and 4 in full; Table 2 as far as the published numbers allow."""
...
def main() -> None:
    ...
    figure1_bias_vs_n(data["bias_grid"], fig_dir, pick(data["bias_grid"]["m"], args.m_fixed, "m"))
    figure2_bias_vs_m(data["bias_grid"], fig_dir, pick(data["bias_grid"]["N"], args.n_fixed, "N"))
    figure3_heatmap(data["bias_grid"], fig_dir)
    figure4_spectrum(results_root, data["neff"], fig_dir)
    figure5_calibration(data["methods"], fig_dir, args.alpha)
    figure6_allocation(data["allocation"], fig_dir)
